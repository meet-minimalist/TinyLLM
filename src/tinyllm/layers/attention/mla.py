...
class MultiHeadLatentAttention(nn.Module):
    ...
    def forward(
        ...
        kv_b = self.kv_b_proj(kv_latent).view(batch, seq_len, H, 2 * D).transpose(1, 2)
        ...
        if self.flash and cu_seqlens is not None and _check_flash_attn():
            ...
            block_mask = create_block_mask(
                _mask_fn, B=None, H=None, Q_LEN=seq_len, KV_LEN=seq_len, device=x.device
            )
            attn_out = flex_attention(q, k, v, block_mask=block_mask, scale=D ** -0.5)
            attn_weights = None
        else:
        ...
        metadata = {
            "q": q, "k": k, "v": v,
            "attn_logits": None, "attn_weights": attn_weights,
            "qkv_out": attn_out, "o_proj_out": o_proj_output,
            "kv_latent": kv_latent,
        }
